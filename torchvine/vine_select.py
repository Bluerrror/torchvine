...
class VinecopSelectorTorch:
    ...
    def _select_pair_copulas(self, tree: _Graph, *, tree_level: int) -> None:
        ...
        for edge_idx, e in enumerate(tree.edges()):
            if e.pc_data is None:
                raise RuntimeError("edge missing pc_data")
            pc_full = e.pc_data

            # Remove incomplete rows pair-by-pair (like vinecopulib).
            mask = torch.isfinite(pc_full).all(dim=1)
            wts = self.controls.weights
            ...
            # Store h-functions for next tree; compute on full data (NaNs propagate).
            e.hfunc1 = cop.hfunc1(pc_full)
            e.hfunc2 = cop.hfunc2(pc_full)
            if e.var_types[1] == "d":
                sub = pc_full.clone()
                sub[:, 1] = sub[:, 3]
                e.hfunc1_sub = cop.hfunc1(sub)
            if e.var_types[0] == "d":
                sub = pc_full.clone()
                sub[:, 0] = sub[:, 2]
                e.hfunc2_sub = cop.hfunc2(sub)
